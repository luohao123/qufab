...
INPUT_SHAPE2 = [2, 4, 64, 64]
SAMPLES = [
    torch.rand(size=INPUT_SHAPE2) for _ in range(128)
...
if direct_test:
    ...
    QS.lsq_optimization_setting.collecting_device = (
        "cuda"  # 缓存数据放在那，cuda 就是放在 gpu，如果显存超了你就换成 'cpu'
    )
    ...
    with ENABLE_CUDA_KERNEL():
        ...
        openvino_results = []
        model = openvino_executor.compile_model(
            model=openvino_executor.read_model(model="model_int8.onnx"),
            device_name="CPU",
        )
        for sample in tqdm(SAMPLES, desc="OPENVINO GENERATEING OUTPUTS"):
            openvino_results.append(model([convert_any_to_numpy(sample)]))
